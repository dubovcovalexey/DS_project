...
if mode == "Пользователь (User ID)":
    ...
    unique_micro_groups = sorted(filtered_df_by_macro['product_category_name_english'].unique().tolist())
...
if mode == "Пользователь (User ID)":
    if selected_user is None:
        ...
        # Модель 2: Честный расчет косинусного сходства векторов признаков лотов
        ...
        # Модель 3: Честный Score Fusion гибрид (без искусственного шума)
        ...
        # Модель 4: Честный прогон башен Two-Tower через PyTorch тензоры
        ...
        # Модель 5: Абсолютно честный инференс CatBoostRanker по матрице признаков
            ...
            if hist_idxs:
                ...
                if cb_model is None:
                    ...
                    # Шаг 2: Извлекаем реальные фичи кандидатов из df_products для построения пула
                    features_list = [
                        'product_weight_g', 'product_length_cm', 'product_height_cm', 'product_width_cm',
